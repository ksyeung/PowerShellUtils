...
$ordinalLibraryNames = @{
    "ws2_32.dll" = $OrdinalNamesws2_32
    "wsock32.dll" = $OrdinalNamesws2_32
    "oleaut32.dll" = $ordinalNamesOleAut32
}

# Generate a lowercase MD5 digest using a string (this isn't available in PS yet)
function Get-MD5FromString {
    param (
        [Parameter(Mandatory=$true)]
        [String]$content
    )
    $myStream = [IO.MemoryStream]::new([byte[]][char[]]$content)
    $digest = Get-FileHash -InputStream $mystream -Algorithm "MD5"

    return $digest.Hash.ToLower()
}

# Return ordinal name by ordinal value
function Get-OrdinalName {
    param (
        [Parameter(Mandatory=$true)]
        [String]$libraryName,
        [Parameter(Mandatory=$true)]
    ...
    if (ordinalLibraryNames.ContainsKey($libraryName)) {
        $name = ords[$libraryName]
    }
    else {
        if (-not $makeName) {
            return "ord$ordinalValue"
        }
    }
}
...
function Get-LibraryName {
    param (
        [Parameter(Mandatory=$true)]
        [String]$moduleName,
        [String[]]$extensions = @("ocx", "sys", "dll")
    )

    $libraryName = $moduleName.ToLower()

    $parts = $libraryName -split "\.", 2

    if ($parts.Length -gt 1 -and $extensions -contains $parts[1]) {
        $libraryName = $parts[0]
    }
...
function Get-FunctionName {
    ...
    if (-not $entryFunctionName) {
        $functionName = Get-OrdinalName -libraryName $libraryName -ordinalValue $entryOrdinal -makeName $true
        if (-not $functionName) {
            throw "PEFormatError: Unable to look up ordinal $($libraryName):$(('{0:X4}' -f $entryOrdinal))"
        }
    } else {
        $functionName = $entryFunctionName.ToLower()
    }

    if ($functionName -is [byte[]]) {
        $functionName = [System.Text.Encoding]::UTF8.GetString($functionName)
    }
...
function Get-ImpHash {
    ...
    $pe = Get-PE -FilePath $path
    $importStrings = @()

    foreach ($entry in $pe.imports) {
        $libraryName = Get-LibraryName -moduleName $entry.ModuleName
        $functionName = Get-FunctionName -entryFunctionName $entry.FunctionName -libraryName $libraryName -entryOrdinal $entry.Ordinal

        if ($functionName) {
            $importStrings += "$libraryName.$functionName"
        }
    }

    $digest = Get-MD5FromString -content ($importStrings -Join ",")
